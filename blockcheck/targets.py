...
from config import app_dir, get_store
...
def _iter_override_candidates(filename: str, filepath: str | Path | None = None) -> list[Path]:
    ...
    base = app_dir()
    candidates = [
        base / "blockcheck" / "data" / filename,
        base / "user" / "blockcheck" / filename,
    ]
...
def load_user_domains() -> list[str]:
    """Load user-added custom domains from the settings store."""
    try:
        domains = get_store().get("blockcheck", "user_domains", [])
...
def save_user_domains(domains: list[str]) -> None:
    """Save user custom domains to the settings store."""
    try:
        ...
        for d in domains:
            ...
            if d and d not in seen:
                seen.add(d)
                unique.append(d)
        get_store().set("blockcheck", "user_domains", unique)
...
def get_catalog_target_domains(limit: int | None = None) -> list[str]:
    """Домены целей из hosts-каталога (включённые сервисы).

    Список следует за доменными именами каталога: при обновлении БД цели
    скана обновляются автоматически. При любой ошибке чтения — пустой
    список (скан не должен падать из-за каталога).
    """
    try:
        from hosts.catalog_repository import catalog_domain_names
        domains = catalog_domain_names()
    except Exception:  # noqa: BLE001
        logger.warning("Failed to load catalog target domains", exc_info=True)
        return []
    if limit and int(limit) > 0:
        return domains[: int(limit)]
    return domains


def get_default_scan_domains(
    *,
    catalog_limit: int | None = None,
    include_user: bool = True,
) -> list[str]:
    """Полный список доменов скана: встроенные + каталог hosts + пользовательские.

    Порядок стабилен: сначала встроенный список, затем домены каталога,
    затем пользовательские. Дубликаты удаляются.
    """
    out: list[str] = []
    seen: set[str] = set()
    for host in get_default_https_targets_domains():
        if host and host not in seen:
            seen.add(host)
            out.append(host)
    for host in get_catalog_target_domains(limit=catalog_limit):
        if host and host not in seen:
            seen.add(host)
            out.append(host)
    if include_user:
        for host in load_user_domains():
            if host and host not in seen:
                seen.add(host)
                out.append(host)
    return out
